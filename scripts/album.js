...
window.onload = function() {
    ...
    var clickHandler = function(targetElement) {
        ...
        // if song is null, set button to pause and set current playing song to song number
        } else if (currentlyPlayingSong ===                 songItem.getAttribute('data-song-number')) {
            songItem.innerHTML = playButtonTemplate;
            currentlyPlayingSong = null;
        // revert back to play button after another click
        } else if (currentlyPlayingSong !== songItem.getAttribute('data-song-number')) {
            var currentlyPlayingSongElement = document.querySelector('[data-song-number="' + currentlyPlayingSong + '"]');
            currentlyPlayingSongElement.innerHTML = currentlyPlayingSongElement.getAttribute('data-song-number');
            songItem.innerHTML = pauseButtonTemplate;
            currentlyPlayingSong = songItem.getAttribute('data-song-number');
        // if clicked song isn't active, set it to pause button
        }
    ...
    for (var i = 0; i < songRows.length; i++) {
        songRows[i].addEventListener('mouseleave', function(event) {
            // #1 - cache song item and song number in variables
            var songItem = getSongItem(event.target);
            var songItemNumber = songItem.getAttribute('data-song-number');
            // #2 - checks the item the mouse is leaving is not the current song
